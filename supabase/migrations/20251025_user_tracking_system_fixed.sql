-- User Tracking System for Dashboard (Fixed - Idempotent)
...
CREATE TABLE IF NOT EXISTS workout_logs (
  ...
  -- Notes
  notes TEXT,
  feeling VARCHAR(50), -- terrible, tired, okay, good, great
...
CREATE TABLE IF NOT EXISTS ai_meal_plans (
  ...
  -- Generated Meals
  meals JSONB NOT NULL, -- [{meal_type, name, ingredients, calories, macros}, ...]
...
CREATE TABLE IF NOT EXISTS ai_workout_plans (
  ...
  equipment VARCHAR(100), -- bodyweight, dumbbells, gym, etc.
  focus_area VARCHAR(100), -- full_body, upper_body, lower_body, cardio, etc.

  -- Generated Workouts
  workouts JSONB NOT NULL, -- [{name, sets, reps, duration, instructions}, ...]
...
DROP POLICY IF EXISTS "Users can manage their meal logs" ON meal_logs;
DROP POLICY IF EXISTS "Users can manage their workout logs" ON workout_logs;
DROP POLICY IF EXISTS "Users can manage their meal plans" ON ai_meal_plans;
DROP POLICY IF EXISTS "Users can manage their workout plans" ON ai_workout_plans;
DROP POLICY IF EXISTS "Users can view their achievements" ON user_achievements;
DROP POLICY IF EXISTS "Users can view their streaks" ON user_streaks;
DROP POLICY IF EXISTS "Users can view their activity feed" ON activity_feed;
DROP POLICY IF EXISTS "Users can manage their reminders" ON user_reminders;
...
DROP TRIGGER IF EXISTS update_ai_meal_plans_updated_at ON ai_meal_plans;
DROP TRIGGER IF EXISTS update_ai_workout_plans_updated_at ON ai_workout_plans;
...
-- Success message
SELECT 'User tracking system tables created/updated successfully! ✅' as status;
