-- =====================================================
-- FIX DUPLICATES AND ADD ONLY GUIDES + MEAL PLANS
-- =====================================================

-- STEP 1: Remove duplicate resources (keep only the first occurrence)
-- =====================================================
...
-- STEP 2: Add ONLY the 20 PDF Guides (not the articles)
-- =====================================================

INSERT INTO educational_resources (title, description, content, resource_type, category, tags, is_public, file_url, view_count) VALUES
...
-- STEP 3: Create tier-specific meal plan templates table
-- =====================================================
...
DROP POLICY IF EXISTS "Authenticated users can view tier default meal plans" ON tier_default_meal_plans;
...
-- STEP 4: Insert meal plans for each tier
-- =====================================================

-- Delete existing plans to avoid conflicts
DELETE FROM tier_default_meal_plans WHERE tier_name IN ('Base', 'Premium', 'Ultimate', 'Elite');
...
-- BASE TIER (FREE) - Complete 7-day meal plan
...
-- PREMIUM TIER
('Premium', 'Performance Optimization Plan', 'Advanced 7-day plan optimized for body composition with macro cycling.', 7, 2200, 180, 220, 70,
'{
  "overview": "Performance-focused plan with nutrient timing and macro cycling for optimal results.",
  "features": ["Macro cycling", "Nutrient timing", "Pre/post workout nutrition", "Higher protein"]
}'::jsonb),

-- ULTIMATE TIER
('Ultimate', 'Elite Transformation Plan', 'Customized 14-day plan with carb cycling and strategic refeeds.', 14, 2400, 200, 240, 75,
'{
  "overview": "Advanced plan with carb cycling, strategic refeeds on days 7 and 14.",
  "features": ["Carb cycling", "Strategic refeeds", "Training/rest day split", "Performance optimization"]
}'::jsonb),

-- ELITE TIER
('Elite', 'Concierge Custom Nutrition Program', 'Fully personalized 30-day rotating plan with medical optimization.', 30, 2500, 210, 250, 80,
'{
  "overview": "Premium customized plan created by certified nutritionists and medical professionals.",
  "features": ["30-day rotating", "Chef-quality recipes", "Biomarker optimization", "DNA integration", "Weekly adjustments"]
}'::jsonb);


-- STEP 5: Create auto-assignment triggers
-- =====================================================

-- Function for new subscribers
...
-- SUCCESS MESSAGE
SELECT
  '✅ DUPLICATES REMOVED, GUIDES ADDED, MEAL PLANS CONFIGURED!' as status,
  (SELECT COUNT(*) FROM educational_resources WHERE resource_type = 'article') as articles,
  (SELECT COUNT(*) FROM educational_resources WHERE resource_type = 'pdf') as guides,
  (SELECT COUNT(*) FROM tier_default_meal_plans) as meal_plan_tiers;
