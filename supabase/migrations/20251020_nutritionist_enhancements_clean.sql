-- Enhanced Nutritionist Dashboard Database Schema (Clean Version)
...
-- Messages (check if exists first)
DO $$
BEGIN
...
-- RLS Policies (Drop existing ones first if they exist)

-- Client Progress
ALTER TABLE client_progress ENABLE ROW LEVEL SECURITY;
DROP POLICY IF EXISTS "Nutritionists can manage their clients' progress" ON client_progress;
...
DROP POLICY IF EXISTS "Users can view their own health data" ON client_health_data;
CREATE POLICY "Users can view their own health data" ON client_health_data
  FOR SELECT USING (auth.uid() = client_id);

DROP POLICY IF EXISTS "Nutritionists can view their clients' health data" ON client_health_data;
...
DROP POLICY IF EXISTS "Clients can update their own health data" ON client_health_data;
...
DROP POLICY IF EXISTS "Clients can manage their habits" ON client_habits;
CREATE POLICY "Clients can manage their habits" ON client_habits
  FOR ALL USING (auth.uid() = client_id);

ALTER TABLE habit_logs ENABLE ROW LEVEL SECURITY;
DROP POLICY IF EXISTS "Clients can manage their habit logs" ON habit_logs;
...
DROP POLICY IF EXISTS "Anyone can view public resources" ON educational_resources;
CREATE POLICY "Anyone can view public resources" ON educational_resources
  FOR SELECT USING (is_public = true OR auth.uid() = nutritionist_id);

DROP POLICY IF EXISTS "Nutritionists can manage their resources" ON educational_resources;
...
DROP TRIGGER IF EXISTS update_client_health_data_updated_at ON client_health_data;
...
DROP TRIGGER IF EXISTS update_meal_plans_v2_updated_at ON meal_plans_v2;
