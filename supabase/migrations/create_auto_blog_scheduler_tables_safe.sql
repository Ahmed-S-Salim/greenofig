-- Auto Blog Scheduler Database Tables (Safe Version)
...
-- Drop and recreate trigger to ensure it exists correctly
...
-- Insert sample topics only if queue is empty
DO $$
BEGIN
    IF NOT EXISTS (SELECT 1 FROM blog_content_queue LIMIT 1) THEN
        ...
        RAISE NOTICE '✅ Inserted 10 starter topics into queue';
    ELSE
        RAISE NOTICE 'ℹ️  Queue already has topics - skipping insert';
    END IF;
END $$;
...
SELECT
    ...
    COUNT(*) FILTER (WHERE status = 'generated') as generated_topics
FROM blog_content_queue;
