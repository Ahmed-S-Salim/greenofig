-- Messaging System for User-Nutritionist-Admin Communication
-- Allows users to message nutritionists/admins and vice versa
...
CREATE INDEX IF NOT EXISTS idx_message_notifications_user ON message_notifications(user_id, is_read, created_at DESC);
...
DROP POLICY IF EXISTS "Senders can edit their messages" ON messages;
DROP POLICY IF EXISTS "Users can view their notifications" ON message_notifications;
DROP POLICY IF EXISTS "Users can update their notifications" ON message_notifications;
...
CREATE POLICY "Senders can edit their messages" ON messages
...
BEGIN
  ...
  IF NEW.sender_id = v_user_id THEN
    ...
    INSERT INTO message_notifications (user_id, message_id, conversation_id, notification_type)
